...
4. Підвищення привілеїв (Root Flag)
    ...
    * Автентифікація CouchDB: Ніколи не залишайте CouchDB відкритим для зовнішньої мережі без налаштованого адміністраторського пароля (`require_valid_user = true` у файлі `local.ini`).
    * Безпека Docker API: Не виводьте Docker API (`2375`) на мережеві порти без використання взаємної TLS-автентифікації (mTLS). Доступ до сокета Docker еквівалентний повному root-доступу на хост-системі.
