...
2. Точка входу (Initial Access / Foothold)
  ...
  Після цього пробую залити веб-шелл p0wny взятий з https://www.revshells.com/ , але щось не пішло. Пробую залити звичайний реверс-шелл PentestMonkey та запускаю слухача.
...
3. Підвищення привілеїв (Privilege Escalation)
  ...
  Переглядаючи файли користувача, знаходжу якісь скрипти
  
  ![scripts](./img/scripts_1.png)

  Завантажую pspy та дивлюсь процеси. Трохи чекаю та бачу що скрипт `planner.sh`, що запускається від імені `root`.
  ...
  Bash:
  `echo "cp /bin/bash /tmp/ && chmod +s /tmp/" > /etc/print.sh`
